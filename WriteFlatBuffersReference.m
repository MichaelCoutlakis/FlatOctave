...
VT = struct("Offsets", [4, 6, ], "Fields", {{"m_Int32";"m_String";}});
N = length(VT.Offsets);
lenVT = 4 + N * 2;
...
if(VT.Offsets(2) ~= 0)
  offsVT(2) = length(Buf);
  Buf = [Buf, WriteString(TestMessageT.m_String')]
  
end

# Write the VT:
BufVT(1:2) = WriteUint16(lenVT)
BufVT(3:4) = WriteUint16(4 + lenInlineData)  # 4 is length of VTO, included in VT object size
for(idxElem = 1:N)
...
FlatBuf = [WriteUint32(4 + lenPad + lenInner), WriteUint32(offRT), BufVT, Buf]
